...
abstract public class ID3V2Frame implements ID3Subject, ID3Visitable
{
        ...
        throws ID3Exception
    {
        ...
        try
        {
            ...
            /*
             * CEP3
             * This is bug in the Stock JID3 code that I had to fix. When not in strict mode, invalid frames
             * should be ignored. I had to add the ELSE clause so that it was properly ignored.
             */
            if (( ! sFrameId.matches("[A-Z0-9]+")) && ID3V2Tag.usingStrict())
            {
            	// Throw an exception if the frame is not valid and we are using strict mode.
                throw new InvalidFrameID3Exception("Invalid frame id [" + sFrameId + "].");
            }
            else if( sFrameId.matches("[A-Z0-9]+") )
            {
            	// This appears to be a valid frame, so read it.
